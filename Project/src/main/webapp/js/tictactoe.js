...
var TIED_STATUS = "The controller is tied.";
var WEBSOCKET_CLOSED_STATUS = "Connection has been closed.";
var OPPONENT_RESIGNED = "Your opponent has resigned the game.";
...
var MESSAGE_OPPONENT_RESIGNED = "opponent_resigned";
...
$(document).ready(function () {
    ...
    $(".grid").click(function () {
        ...
        if (gameEnded == false && yourTurn == true) {
            // Stop processing clicks and invoke sendMessage().
            yourTurn = false;
            sendMessage(this.id);
            // Add the X or O to the controller board and update status.
    ...
    ws.onmessage = function (event) {
        ...
        if (message.type === MESSAGE_OPPONENT_UPDATE) {
            // Show their turn info on the controller board.
        ...
        /* The initial turn indicator from the server. Determines who starts
            the controller first. Both players wait until the server gives the OK
        ...
        if (message.type === MESSAGE_OPPONENT_RESIGNED) {
            $('#status').text(OPPONENT_RESIGNED);
            gameEnded = true;
        }
    }
